...
The last three were found on the **published** page rather than in the checks, which is worth saying plainly: the first version of this demo shipped with both line charts blank and the verification passing. What that cost, and what now stops it happening again, is under "Checking it" below.
...
**F-FX-6 — a `line` chart on a band scale draws an EMPTY path, silently.** Bind a `line` chart's `x` to a column the charts module resolves as a category and the chart produces a `<path>` with an empty `d` attribute: no line, no warning, no error, no thrown exception. `chart.data()` is no help — it reports the series correctly, 380 points each carrying a real `y`, because the binding succeeded and only the drawing is missing. The axis frame and its tick labels are drawn as usual, so the chart looks finished and is simply blank. Repro without this demo: 7,094 rows of `{ date: 'YYYY-MM-DD', v: number }`, a column declared `{ id: 'date', field: 'date', type: 'dateString' }`, and `{ type: 'line', x: 'date', y: 'v' }` — `data().kind` is `'category'`, the longest path `d` is 0 characters, and the chart emits **7,099 `<text>` tick labels**, one per row. The same column with `type: 'date'`, or with no type at all, resolves as `'time'` and draws a 14,657-character path. Note the second half of that: `dateString` is a documented temporal type and it is the one that loses the time scale. Wanted: a band scale of thousands of categories refused or warned about for a line chart rather than rendered empty, one tick label per *tick* rather than per row, and `dateString` treated as temporal as `date` is.

This one bit the demo rather than only being noticed by it. The table's Date column was `field: 'date'` with no type — the plain string — which on this grid resolved to a category axis, and **both line charts on the published page were blank**. That was a modelling mistake on the demo's side and it is fixed: the column is now a real `type: 'date'` over the epoch-millisecond field, which is how a date should have been declared in the first place. The finding stands regardless of the demo's mistake, because the failure mode is silence: nothing anywhere said the chart had not drawn.
...
The first version of this demo shipped with **both line charts blank** and all 383 checks passing. That is worth being precise about, because the check that let it through looked reasonable.

It asked two things of every chart: that `chart.data()` reported points carrying a measure, and that the chart box held more than two of `rect, circle, path, line, polyline, text`. Both were true of a blank chart. The data was bound correctly — 380 points, each with a real `y` — because the binding was never the problem. And the DOM held nine nodes: two axis rectangles, six tick labels, and the one `<path>` that should have been the line, present but with an empty `d`. **Axis furniture cleared the bar on its own.** The check was measuring that the charts module had produced some DOM, and calling it "the chart drew".

What it does now is measure the geometry a chart draws its *data* with, separately from its frame:

- no chart may leave a `<path>` with an empty `d` behind;
- every chart bound to values must have either a path whose `d` is at least 64 characters — a real series, not a zero line or a tick — or more than a handful of data marks, or a canvas;
- a `line`, `area`, `step` or `scatter` bound to an x column must be on a **continuous** axis: `data().kind` of `category` now fails, because a band scale of thousands of categories is what produced the empty path, and it also emits one tick label per row rather than per tick;
- the same geometry rule applies to all seven plots on the Statistics tab, on every pass.

Six of them fail on the old code — chart 0 with a path of nought characters, chart 1 with only the 18-character dashed zero line, and both reporting `kind: category` — and all of them pass on the new. The count went from 383 to 409.
